...
public class TacticalRadarUI : MonoBehaviour
{
    ...
    public float radarSize = 200f;  // Diamètre à l'écran en pixels
    ...
    public void RegisterUnit(Transform unit, bool isEnemy) { }
    ...
    void OnGUI()
    {
        ...
        float uiScale = Mathf.Clamp(Screen.width / 480f, 1.25f, 2.0f);
        ...
        float rY = pad;
        Rect radarRect = new Rect(rX, rY, radarSize, radarSize + 22f);
        ...
        // Bas Gauche
        GUI.DrawTexture(new Rect(rX, rY + radarSize + 22f - bThick, cornerLen, bThick), borderTex);
        GUI.DrawTexture(new Rect(rX, rY + radarSize + 22f - cornerLen, bThick, cornerLen), borderTex);
        // Bas Droite
        GUI.DrawTexture(new Rect(rX + radarSize - cornerLen, rY + radarSize + 22f - bThick, cornerLen, bThick), borderTex);
        GUI.DrawTexture(new Rect(rX + radarSize - bThick, rY + radarSize + 22f - cornerLen, bThick, cornerLen), borderTex);
        ...
        GUI.Label(new Rect(rX, rY + 4, radarSize, 18), "🛰️ RADAR TACTIQUE 90M", headerStyle);

        Vector2 center = new Vector2(rX + radarSize * 0.5f, rY + radarSize * 0.5f + 4f);
        float radius = (radarSize * 0.5f) - 18f;
