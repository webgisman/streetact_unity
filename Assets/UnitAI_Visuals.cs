...
public partial class UnitAI
{
    ...
    /// <summary>
    /// Crée un traceur lumineux pour simuler le projectile.
    /// </summary>
    private void SpawnTracer(Vector3 start, Vector3 end, bool isTankShot)
    {
        ...
        // Un obus de char est énorme et lent, une balle est fine et très rapide
        ...
        tracerMat.color = isTankShot ? new Color(1f, 0.5f, 0f) : Color.yellow; // Obus orange vif
        line.material = tracerMat;
        
        line.SetPosition(0, start);
        line.SetPosition(1, end);
        
        Destroy(lineObj, isTankShot ? 0.2f : 0.06f); // L'obus reste visible plus longtemps à l'écran
    }
    ...
    private void SpawnBloodEffect(Vector3 position, Vector3 direction)
    {
        ...
        main.startColor = new Color(0.6f, 0f, 0f, 1f); // Rouge sombre
        ...
        ps.Play();
        Destroy(bloodObj, 1.5f); // Nettoyer après 1.5s
    }
    ...
    private void SpawnSparksEffect(Vector3 position, Vector3 direction)
    {
        ...
        main.startColor = new Color(1f, 0.7f, 0.1f, 1f); // Orange vif métallique
    ...
    private void SpawnBlackSmoke()
    {
        ...
        main.startColor = new Color(0.1f, 0.1f, 0.1f, 0.8f); // Fumée noire épaisse
        main.maxParticles = 100;
        main.playOnAwake = true;
